...
class JsonToCppStruct:
    ...
    def _generate_serializer_functions(self, struct_name: str, fields: List[str]) -> str:
        ...
        # from_json 생성 (키 누락 방지를 위해 at() 사용)
    ...
    def convert(self, json_str: str, root_name: str = "Root") -> str:
        ...
        if isinstance(data, list):
            ...
            return "// 유효한 JSON 객체 또는 배열이 아닙니다."
...
if __name__ == "__main__":
    ...
    # 1. 매크로 방식 생성 (기본값, 코드가 간결함)
    converter_macro = JsonToCppStruct(use_macro=True)
    print("// === 1. 매크로 방식 ===")
    print(converter_macro.convert(sample_json, root_name="UserResponse"))

    print("\n" + "="*50 + "\n")

    # 2. 명시적 to_json / from_json 함수 방식 생성
    converter_func = JsonToCppStruct(use_macro=False)
    print("// === 2. 함수 직접 정의 방식 ===")
    print(converter_func.convert(sample_json, root_name="UserResponse"))
