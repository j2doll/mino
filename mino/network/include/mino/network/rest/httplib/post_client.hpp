...
namespace mino::network::rest::httplib {
    ...
    public:
        ...
        {
            ok, // 성공 (HTTP 2xx)
            curl_timeout, // 타임아웃 오류 (호환용 이름 유지)
            curl_ssl_error, // SSL 인증 오류
            curl_network_error, // 네트워크 연결 오류
            curl_other_error, // 기타 오류
        ...
        struct response
        {
            ...
            int          curl_error_code = 0; // (호환용) 오류 코드
            ...
            bool is_success() const // HTTP 2xx이면서 오류가 없는 경우를 성공으로 간주
            {
