#include "mino/network/redis/basic_redis_client.hpp"
...
namespace mino::network::redis {
    ...
    void basic_redis_client<transport_t>::configure_tls(const redis_tls_config& config) {
        if constexpr (std::is_same_v<transport_t, mino::network::tls::tls_client>) {
    ...
    template class basic_redis_client<mino::network::tcp::tcp_client>; 
        template class basic_redis_client<mino::network::tls::tls_client>;  
