...
#include "mino/network_openssl/rest/patch_client.hpp"

namespace mino::network_openssl::rest {

    struct patch_client::impl
    {
        impl() = default;
        ~impl() = default;
    ...
    patch_client::patch_client() : impl_(std::make_unique<impl>()) {}
    patch_client::~patch_client() = default;

    void patch_client::set_server(const std::string& scheme, const std::string& host, long port, const std::string& path)
    {
    ...
    void patch_client::set_headers(const headers& headers) { headers_ = headers; }

    void patch_client::set_timeout_ms(long timeout_ms)
    {
    ...
    patch_client::response patch_client::patch(const std::string& body_str)
    {
        response resp;
        ...
        auto result = impl_->client_impl->Patch(req_path, h, body_str, content_type);
        if (!result) {
    ...
    patch_client::result_code patch_client::patch(const std::string& body_str, response& out_resp) noexcept
    {
        out_resp = patch(body_str);
