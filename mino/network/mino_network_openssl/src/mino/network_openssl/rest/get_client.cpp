...
namespace mino::network_openssl::rest {
    ...
    get_client::get_client() : impl_(std::make_unique<impl>()) {}
    get_client::~get_client() = default;

    void get_client::set_server(const std::string& scheme, const std::string& host, long port, const std::string& path)
    {
    ...
    void get_client::set_headers(const headers& headers) { headers_ = headers; }
