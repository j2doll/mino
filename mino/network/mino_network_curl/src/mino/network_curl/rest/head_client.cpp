...
#include "mino/network_curl/rest/head_client.hpp"
...
namespace mino::network_curl::rest {
    ...
    size_t head_client::header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
    {
    ...
    bool head_client::set_server(const std::string& scheme,
    ...
    void head_client::set_headers(const headers& headers)
    {
        headers_ = headers;
    }

    bool head_client::set_timeout_ms(long timeout_ms)
    {
        if (timeout_ms <= 0)
            return false;
    ...
    void head_client::set_ignore_ssl_errors(bool ignore)
    {
        ignore_ssl_errors_ = ignore;
    }

    std::vector<std::string> head_client::build_header_lines(const headers& headers)
    {
    ...
    head_client::http_status head_client::to_http_status(long code)
    {
        switch (code) {
    ...
    std::string head_client::build_url(const query_params& query_params)
    {
    ...
    head_client::response head_client::head(const query_params& query_params)
    {
        response resp;
        if (!curl_) {
        ...
        curl_easy_setopt(curl_, CURLOPT_NOBODY, 1L); // HEAD request mode
    ...
    head_client::result_code head_client::classify(const response& r)
    {
        if (!r.error.empty()) {
    ...
    head_client::result_code
        head_client::head(const query_params& query_params, response& out_resp) noexcept
    {
        out_resp = head(query_params);
