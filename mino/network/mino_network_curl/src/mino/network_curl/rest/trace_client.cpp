...
#include "mino/network_curl/rest/trace_client.hpp"
...
namespace mino::network_curl::rest {
    ...
    size_t trace_client::write_callback(char* ptr, size_t size, size_t nmemb, void* userdata)
    {
    ...
    size_t trace_client::header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
    {
    ...
    bool trace_client::set_server(const std::string& scheme,
    ...
    void trace_client::set_headers(const headers& headers)
    {
        headers_ = headers;
    }

    bool trace_client::set_timeout_ms(long timeout_ms)
    {
        if (timeout_ms <= 0)
            return false;
    ...
    void trace_client::set_ignore_ssl_errors(bool ignore)
    {
        ignore_ssl_errors_ = ignore;
    }

    std::vector<std::string> trace_client::build_header_lines(const headers& headers)
    {
    ...
    std::string trace_client::build_url(const query_params& query_params)
    {
    ...
    trace_client::response trace_client::trace(const query_params& query_params)
    {
        response resp;
        if (!curl_) {
        ...
        curl_easy_setopt(curl_, CURLOPT_CUSTOMREQUEST, "TRACE");
    ...
    trace_client::result_code
        trace_client::trace(const query_params& query_params, response& out_resp) noexcept
    {
        out_resp = trace(query_params);
