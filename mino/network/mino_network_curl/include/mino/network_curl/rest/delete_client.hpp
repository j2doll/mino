...
namespace mino::network_curl::rest {
    ...
    public:
        ...
        // DELETE 요청 실행 (C++ 예약어 delete 회피: del() 및 delete_request() 제공)
        ...
        response delete_request(const query_params& query_params = {}, const std::string& body = "")
        {
            return del(query_params, body);
        }

        result_code delete_request(const query_params& query_params, const std::string& body, response& out_resp) noexcept
        {
            return del(query_params, body, out_resp);
        }
