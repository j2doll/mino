...
void test_constructors_and_basic_methods() {
    ...
    bit_array b(bytes, 16); // 16비트 크기로 bytes 값으로 초기화
    assert(b.size() == 16);
    assert(b.data() == bytes);

    // bits 매개변수가 0일 때 (자동으로 byte 크기 * 8 비트로 계산)
    bit_array c;
    c.from(bytes, 0); // 값이 0인 바이트 배열로 초기화
    assert(c.size() == 16);
    assert(c.data() == bytes);
...
void test_set_bytes_and_bits() {
    ...
    // set_bytes
    arr.set_bytes(3); // 지정된 바이트 크기(3바이트)로, 0으로 채운 배열 설정
    assert(arr.size() == 24);
...
void test_to_array() {
    ...
    bit_array arr(bytes, 8); // 8비트 크기로 초기화

    std::vector<bool> expected = { true, false, true, false, false, true, false, true };
    std::vector<bool> result = arr.to_array(); // 8비트 크기이므로 8개의 bool 값 반환
...
void test_get() {
    ...
    // 0xAA (1010 1010), 0x55 (0101 0101) -> 연속 16비트: 1010 1010 0101 0101
    //                                            오프셋: 0123 4567 8901 2345
    std::vector<uint8_t> bytes = { 0xAA, 0x55 };
    bit_array arr(bytes, 16);

    // 4번째 비트 오프셋부터 8비트 추출
    // 오프셋 4~11: 1010 (AA 하위4비트) + 0101 (55 상위4비트) => 1010 0101 (0xA5)
...
void test_merge() {
    ...
    bit_array target(target_bytes, 16); // 16비트 크기로 초기화

    // 소스: 0xFF (1111 1111 - 8비트 크기)
    std::vector<uint8_t> src_bytes = { 0xFF };
    bit_array src(src_bytes, 8); // 8비트 크기로 초기화
...
void test_operator_plus() {
    ...
    // joined = 1010 + 0101 (8비트, 0xA5)
    bit_array joined = a + b; // 1010 0101 = 0xA5
...
//------------------------------------------------------------------------------
// 7. operator~ 테스트 (bitwise NOT 연산자)
//------------------------------------------------------------------------------
void test_operator_not() {
    std::cout << "[RUN] test_operator_not..." << std::endl;

    // 4비트 예: 0xB0 = 1011 0000 (상위 4비트 사용: 1011)
    // 비트wise NOT -> 0100 -> 상위 4비트에 위치: 0100 0000 = 0x40
    bit_array a(std::vector<uint8_t>{ 0xB0 }, 4);
    bit_array not_a = ~a;
    assert(not_a.size() == 4);
    assert(not_a.data().size() == 1);
    assert(not_a.data()[0] == 0x40);

    // 8비트 예: 0xA5 = 1010 0101 -> NOT -> 0101 1010 = 0x5A
    bit_array b(std::vector<uint8_t>{ 0xA5 }, 8);
    bit_array not_b = ~b;
    assert(not_b.data()[0] == 0x5A);

    std::cout << "  [PASS] operator~ tested successfully.\n";
}

//------------------------------------------------------------------------------
// 8. operator<<, operator>> (시프트 연산자) 검증
//------------------------------------------------------------------------------
void test_shift_operators() {
    ...
    //                     #### ##<<
    // Left shift 2비트 -> 1001 0100 (0x94)
    bit_array shl = arr << 2;
    assert(shl.size() == 8);
    assert(shl.data()[0] == 0x94);

    //                      >>## ####
    // Right shift 2비트 -> 0010 1001 (0x29)
    bit_array shr = arr >> 2;
    assert(shr.size() == 8);
...
void test_reverser() {
    ...
    // 0xB0 = "1011" 0000 (4비트만 사용: 1011)
    bit_array arr(std::vector<uint8_t>{ 0xB0 }, 4);

    // 반전 후: "1101" 0000 -> 0xD0
    // (인덱스대로 [1,0,1,1] → 뒤집으면 [1,1,0,1])
    // 비트 순서 역전(reversal) ≠ 비트 반전(bitwise NOT).
    // (예): 1011의 비트 반전(보수)은 0100이지만, 역전은 1101
    arr.reverser(); // 반전
    assert(arr.size() == 4);
    assert(arr.data()[0] == 0xD0);

    // 8비트 풀 반전 테스트: 1010 0101 (0xA5) -> 반전 후 1020 0101 (0xA5: 대칭)
    bit_array arr2(std::vector<uint8_t>{ 0xA5 }, 8);
    arr2.reverser();
    assert(arr2.data()[0] == 0xA5);

    // 비대칭 8비트: 1100 0000 (0xC0) -> 반전 후 0000 0011 (0x03)
